...
fixed-point TOML (``peetsfea data 0.3.5_fixed.toml``) instead of a sampled seed.
...
Run from ``run/``:
    ...
    ../.venv/bin/python ../entry/view.py --fixed   # view peetsfea data 0.3.5_fixed.toml
...
FIXED_SUBDIR = "fixed"
DEFAULT_FIXED_TOML = DEFAULT_SOURCE_TOML_PATH  # peetsfea data 0.3.5_fixed.toml
...
def generate_fixed(*, fixed_toml: Path, output_root: Path) -> SampledStep:
    """Export the authored fixed-point TOML (peetsfea data 0.3.5_fixed.toml) to a STEP scene."""
...
def _resolve_config(argv: list[str]) -> ViewConfig:
    ...
    parser.add_argument(
        "--fixed",
        action="store_true",
        help="ignore seeds; generate and view the authored fixed-point TOML (peetsfea data 0.3.5_fixed.toml)",
    )
