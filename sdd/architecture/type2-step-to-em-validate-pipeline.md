...
5. Prototype adapter가 metadata ledger와 `imported_object_names`를 합쳐 imported modeled-object ledger entry를 만든다.
6. Post-prototype adapter가 ledger의 future EM mapping role을 읽어 `EmPipelineInput.ready_objects`, `ports`, `context`, `endpoints`를 조립한다.
7. `run_em_pipeline()`가 grouping, series/subtract metadata, radiation boundary, explicit ports, source phase, analysis setup, post template creation을 수행한다.
8. `validate_pipeline()`가 repo-level TX/RX conductor readiness를 검사하고, AEDT `ValidateDesign()`가 design-level validation을 수행한다.
...
- standalone `examples/tx_rect_void/tx_rect_void_coil.toml` is not the future type2 public SSOT.
- historical `examples/type2/type2_non_model_objects.toml` is not the future type2 public SSOT.
