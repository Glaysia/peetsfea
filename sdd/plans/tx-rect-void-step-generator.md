...
- `examples/tx_rect_void/tx_rect_void_coil.toml`의 geometry fields는 `type2.toml`의 `[[modeled_objects]]` entry로 흡수한다.
...
- 제외:
  - `examples/tx_rect_void/tx_rect_void_coil.toml`을 새 public input으로 계속 확장하는 작업
...
- `examples/tx_rect_void/tx_rect_void_coil.toml`은 migration source 또는 test fixture로만 남긴다.
