...
이 다이어그램은 `examples/type2/type2.toml`에서 type2 object-level STEP artifact와 EM validation까지 이어지는 planned flow를 보여준다. 상위 계획은 [[sdd/plans/0.2.22-type2-step-to-em-validate-pipeline]], TOML 단일화 계획은 [[sdd/plans/0.2.22-type2-toml-unification]], 아키텍처 설명은 [[sdd/architecture/type2-step-to-em-validate-pipeline]]다.
...
## Notes
- `examples/type2/type2.toml` is the single planned public authoring input.
