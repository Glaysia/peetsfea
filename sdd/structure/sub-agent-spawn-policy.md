...
이 문서는 이 저장소에서 서브에이전트를 생성할 때 사용할 모델 조합, 생성 실패 시 재시도 순서, 금지 패턴을 고정한다.
...
사용자가 `mini`, `5.4mini`, 또는 `gpt-5.4-mini`를 명시한 경우 아래 예시의 `model`은 `gpt-5.4-mini`로 고정한다.
...
- 기본 단일 서브에이전트 생성의 retry는 한 단계만 허용한다.
