...
- Official active fixtures fix `tx_reference_line.y_usage_ratio` and `tx_reference_line.z_ratio` as singleton ranges, so they stay out of sampled metadata.
...
- Synthetic sampled single-coil `turn_qcount` ranges use quarter-turn active bounds such as `[true, 4, 12, 9]` / `RangeSpec(4.0, 12.0, count=9)`.
...
- RX sampled `turn_qcount` manifest values are members of `{4, 5, 6, 7, 8, 9, 10, 11, 12}`.
...
- Synthetic source TOML fixtures fix `modeled_objects.tv_aluminum_plate.sheet_present` disabled so manifest metadata excludes the TV aluminum sheet presence owner.
...
- `sampled_owner_paths` must include every active `count > 1` range owner, including the non-modeled TX Z gap, both `terminal_start` owners, and both `void_stack_present` owners, and exclude fixed singleton reference-line ranges.
- `sampled_owner_paths` must exclude fixed TV aluminum `sheet_present`, and sampled TOML must preserve its disabled singleton integer range.
