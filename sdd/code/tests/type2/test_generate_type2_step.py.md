...
- TX/RX exact no-shoe baseline contract (`43` bodies each)
...
- interleaved `PET/PSA -> ferrite -> air` flattened ferrite-member ordering
...
- bridge contract regression:
  ...
  - `tx_bridge_s*` / `rx_bridge_s*`는 wall/coil copper turns 및 notched slab bodies(`pcb_wall`, `pcb_coil`, `stack_pet_psa_u*`, `stack_ferrite_u*`, `stack_air_u*`)와 positive-volume intersection이 없어야 한다.

## 변경 시 주의점
- active example role drift와 exact-name order drift를 같은 테스트 층에서 잡아야 한다.
- exact body name/order/count(`43`) contract는 유지한 채, bridge geometry invariant만 강화해야 한다.
