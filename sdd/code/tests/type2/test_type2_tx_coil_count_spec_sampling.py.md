...
- Status: planned
- Related plan: [[sdd/plans/0.2.22-type2-tx-plate-stack-parallel-array]], [[sdd/plans/0.2.22-type2-tx-actual-region-non-model-sampling]], [[sdd/plans/0.2.22-type2-tx-actual-region-pcb-non-model]]
...
- Fixture TOML uses active type2 schema v8 and includes fixed `tx_region_actual` and `tx_region_actual_stack_space` non-model owners, including fixed `tilt_enabled`, so this test can focus on TX plate-stack sampled owners.
