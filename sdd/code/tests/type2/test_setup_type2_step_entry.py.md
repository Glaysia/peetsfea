...
- Generic TX output variable, generic TX modeled role, TX sampled-owner block이 active example surface로 돌아오면 실패한다.
- Geometry-only `tx_inner_single_coil` is allowed as retained modeled geometry and must keep fixed zero underlay.
...
- active modeled objects must contain geometry-only `tx_inner_single_coil` plus RX modeled object(s), and no generic
  TX modeled role.
