...
- active sweep samples `tx_reference_line.y_usage_ratio = [false, 0.2, 1.0, 85]` and
  `tx_reference_line.z_ratio = [false, 0.75, 1.0, 65]` instead of reusing the fixed singleton guide values.
...
- active sweep examples assert TX inner and RX `turn_qcount` ranges as `[true, 3, 7, 5]`; fixed examples remain unchanged at `[true, 3, 3, 1]` for TX inner and `[true, 4, 4, 1]` for RX.
...
- active sweep exposes `non_model_objects.tx_region.z_gap_from_rx_plane_mm = [false, 45.0, 130.0, 37]` and keeps `modeled_objects.tv_aluminum_plate.sheet_present = [true, 0, 1, 2]` plus `tx_reference_line.y_usage_ratio` and `tx_reference_line.z_ratio` as active sampled owners, setting the active sampled-owner count to 18.
...
- fixed TX inner `turn_qcount` must stay at singleton quarter-turn count `3`, fixed RX must stay at `4`, and the official sweep range must stay `[true, 3, 7, 5]`.
...
- active sweep sampled-owner paths must include `non_model_objects.tx_region.z_gap_from_rx_plane_mm`, `tx_reference_line.y_usage_ratio`, `tx_reference_line.z_ratio`, `tv_aluminum_plate.sheet_present`, both `terminal_start` owners, and both `void_stack_present` owners, and have length 18.
