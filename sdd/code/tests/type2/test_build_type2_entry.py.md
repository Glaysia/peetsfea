...
- build helper parser fixture that includes `tx_rect_void_columns` is expected to fail under reset-aware parser/runtime contracts, replacing legacy terminal-stub fixed-value expectations.
- setup-ready/build path fails fast for geometry-only `tx_rect_void_columns` modeled roles with a clear unsupported-role message before step-artifact export and before runner-side source/port setup.
