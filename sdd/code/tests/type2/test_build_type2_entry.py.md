...
- RX effective sampled owners are `outer_x_usage_ratio`, `outer_y_usage_ratio`, `turn_count`, `metal_fill_factor`; `layer_count`, full-backing `underlay_repeat_count`, and removed `void_*` fields are excluded from `design_variables`
