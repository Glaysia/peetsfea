...
- `tx_rect_void_columns` remains unsupported for setup-ready even when a modeled entry carries
  `terminal_metadata.kind = "parallel_collector_tabs"` or `terminal_metadata.kind = "series_collector_tabs"`; the role gate must fail before any HFSS attach or port setup, and coverage keeps both rejection cases explicit.
