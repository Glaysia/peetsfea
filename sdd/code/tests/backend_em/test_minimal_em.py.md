...
- Outputs: assertions for import, explicit all-body material assignment, visual state assignment, mesh, port, restored setup policy, AEDT-compatible sweep payload, analysis, report, save, solve, and release calls.
- Canonical state: fake session call records.
- Invariants: Tx/Rx port names, `Hfss.assign_material()` call records and material readback for every ledger body, restored object colors/transparency, restored `2000mm` radiation region, setup policy `0.0017`/`22/20/21`/`BasisOrder=0`/lambda-refine off, mesh targets, restored TxRx report variables, and diagnostic report surfaces stay fixed with `Table2` X component `Pass`; setup closes AEDT by default but can leave a GUI session open when explicitly requested.
