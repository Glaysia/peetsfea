...
- Canonical state: the fake ledger identity fields, fake ledger body names, TX/RX semantic port edge endpoint pairs, fake mesh/boundary/setup/report call records, fake output variables, fake modeler edge IDs and endpoint coordinates, fake boundary excitations, real imported ledger including TX under-coil copper, saved `.aedt` path, and result `ports`/`boundary`/`reports` mapping.
