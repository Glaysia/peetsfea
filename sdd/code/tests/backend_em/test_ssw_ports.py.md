...
- Canonical state: the fake ledger body names, TX/RX semantic port edge endpoint pairs, fake mesh/setup/report/boundary call records, fake output variables, fake modeler edge IDs and endpoint coordinates, fake boundary excitations, real imported ledger, saved `.aedt` path, and result `ports`/`reports` mapping.
- Invariants: exactly two port edge specs are required, each port edge spec stores exactly two semantic endpoint pairs, material assignment covers every ledger body including vacuum, copper, FR4, and ferrite roles, semantic ferrite entries are assigned as 0.2.25 `MULL12060ferrite` through dataset import/project material definition/material lookup sync, recorded mesh targets and `Setup1`/`Sweep` payload values are preserved, semantic endpoint pairs match imported copper edges before `AssignLumpedPort`, `AssignLumpedPort` creates `1_T1` and `2_T1`, restored Tx/Rx output variables are created against `Setup1 : Sweep`, reports `Output Variables Table1`, `Table1`, and `Table2` are registered, imported ledger content mirrors the setup result, PyAEDT `False` returns raise instead of logging and continuing, and the integration-marked test must launch real headless AEDT rather than a fake session.
- Fail-fast points: failed `AssignLengthOp`, `InsertSetup`, `InsertFrequencySweep`, `AssignLumpedPort`, output-variable creation, `CreateReport`, missing report registration, missing imported bodies, unresolved semantic copper edge endpoints, failed desktop release, failed headless AEDT startup, failed STEP import, missing excitations, or failed project save.
