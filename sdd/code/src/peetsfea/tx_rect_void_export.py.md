...
- `central_corridor_y_bounds()` accepts caller-supplied copper primitives for testable fail-fast validation and returns the proven local `RectBounds` over the selected void X strip; partial-turn coils may use the realized outer Y boundary on an open side after the final copper-overlap proof.
...
- Central corridor derivation clips copper primitive polygons to the selected void X strip and uses the nearest strict above/below blockers from that clipped area, or the realized outer Y boundary for an open partial-turn side.
- Central corridor derivation fails if the realized void strip has non-positive bounds, the void intersects copper, the derived corridor has non-positive Y extent, or the final corridor rectangle overlaps copper with positive area.
