...
## Canonical state
- TX plate families는 `tx_copper_wall_t*`, `tx_pcb_wall`, `tx_stack_pet_psa`, `tx_stack_ferrite`, `tx_stack_air`, `tx_pcb_coil`, `tx_copper_coil_t*`, `tx_bridge_s*`, `tx_stub_*`를 분류한다.
- RX plate families는 `rx_copper_wall_t*`, `rx_pcb_wall`, `rx_stack_pet_psa`, `rx_stack_ferrite`, `rx_stack_air`, `rx_pcb_coil`, `rx_copper_coil_t*`, `rx_bridge_s*`, `rx_stub_*`를 분류한다.
