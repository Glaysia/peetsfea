...
- Single-coil port edges are resolved from the `single_coil_port_v1` ledger contract.
...
- RX/TX single-coil port sheet edge order follows the contract: `[3] -> [0]` signal, `[1] -> [2]` reference.
...
- Single-coil terminal metadata must not be inferred from sheet/body bounding boxes.
