...
- scene import, ownership partition, style/material application, metadata-driven port-sheet reconstruction, imported ledger object assembly를 수행한다.
...
- modeled object styling 전에 underlay exact-name presence를 보고 `MULL12060ferrite` / `PET_PSA` material setup를 조건부로 정확히 한 번 수행한다.
...
- conductor material ownership과 underlay material ownership을 섞지 않는다. coil conductors는 `copper`, TX underlay ferrite는 `MULL12060ferrite`, PET는 `PET_PSA`, air는 `vacuum`이다.
