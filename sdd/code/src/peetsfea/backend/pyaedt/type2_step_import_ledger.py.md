...
- `tv_aluminum_plate` is a zero-STEP-body modeled sheet role owned by non-modeled member `tv`, with `plane = "YZ"`, `material = "aluminum"`, `model_state = true`, required `sheet_present` of `0` or `1`, canonical `sheet_vertices_xyz`, and no exported body groups or terminal metadata.
...
- `tv_aluminum_plate` ledgers fail immediately if the owner is not `tv`, the plane is not `YZ`, material/model state drift, `sheet_present` is not `0` or `1`, canonical sheet vertices are malformed, the expected body list/count is not empty/zero, groups are non-empty, or terminal metadata is non-empty.
