...
- final import ledger의 plate-stack import된 도체는 `tx_plate_copper`, `rx_plate_copper` 단일 body다.
...
- copper grouping contract는 plate-stack role별 단일 그룹을 사용한다:
  ...
  RX는 `member_names == ['rx_plate_copper']`, TX는 `member_names == ['tx_plate_copper']`.
...
- modeled copper groups는 role별로 정확히 1개만 허용하며 `tx_plate_copper`, `rx_plate_copper`가 반드시 포함되어야 한다.
