...
- Outputs: an AEDT project saved after STEP import, explicit material assignment for every ledger body, non-model state assignment, visual state assignment, recorded `Length1` mesh, `Region_Abs_2000mm` radiation boundary, recorded `Setup1`/`Sweep`, direct edge-based lumped-port calls, terminal excitations `1_T1` and `2_T1`, restored Tx/Rx output variables, and reports `Output Variables Table1`, `Table1`, and `Table2`.
- Canonical state: the port ledger JSON, imported ledger JSON, AEDT object names, mesh/boundary/setup/sweep summaries, report summary, output-variable names, and HFSS excitation names after `AssignLumpedPort`.
