...
- Outputs: an AEDT project saved to the caller-provided identity-derived path, explicit material assignment for every ledger body, non-model state assignment, visual state assignment, recorded `Length1` mesh, `Region_Abs_2000mm` radiation boundary, recorded `Setup1`/`Sweep`, direct edge-based lumped-port calls, terminal excitations `1_T1` and `2_T1`, restored Tx/Rx output variables, reports `Output Variables Table1`, `Table1`, and `Table2`, and imported ledger/result identity fields.
