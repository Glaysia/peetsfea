...
- Upstream metadata producer: [[sdd/code/src/peetsfea/tx_rect_void.py]]

## 역할
- `tx_rect_void` metadata JSON의 `modeled_objects[0]`와 modeled STEP import 결과 `imported_object_names`를 합쳐 single imported modeled-object ledger entry를 만든다.
- 현재 single-coil prototype(`role=tx_single_coil`)만 허용하는 fail-fast adapter 계약을 제공한다.

## 입력 / 출력
- 입력:
  - `modeled_object: dict[str, object]` (from metadata JSON `modeled_objects[0]`)
  - `imported_step_path: Path`
  - `imported_object_names: Sequence[str]`
- 출력:
  - `ImportedModeledObjectEntry` typed contract
  - 필수 필드: `object_id`, `role`, `material`, `model_state`, `step_path`, `canonical_coordinates`, `terminal_metadata`, `imported_object_names`
...
- `modeled_object.role` must be exactly `tx_single_coil`.
- `modeled_object.model_state` must be `True`.
- `modeled_object.step_path`와 `imported_step_path`는 resolved path 기준으로 같아야 한다.
...
## 직접 의존
- 표준 라이브러리: `pathlib`, `typing`, `collections.abc`
