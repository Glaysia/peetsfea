...
- TX inner and TX outer ratio placement must not use physical PCB/copper bbox minimum as the ratio reference. Physical modeled bodies follow the design outer footprint after the frame origin is solved.
...
- TX inner actual-underlay stack bodies, when requested, use `tx_inner_actual_region`/design outer bounds as the X/Y footprint and stack in `-Z` below the actual region.
...
- `RealizedSingleCoilFitEnvelope.design_outer_bounds_*` and `outer_bounds_*` are the resolved design outer box intended for actual-region bounds and for TX ratio placement validation.
...
- `tx_inner_single_coil` also emits a passive YZ void stack when repeat_count > 0. The stack uses realized void world bounds from `fit_envelope.realized.void_bounds` plus `fit_envelope.frame_origin_xyz`, spans Z from `fit_envelope.outer_bounds_min_z` to the explicit `tx_region.max_z` scene input, and uses the same nominal underlay thickness values.
