...
- TX outer scene placement must read `tx_outer_region` creation-time coordinates from the non-model scene; it must not sort or reverse-calculate the sloped prism from downstream modeled bodies.
