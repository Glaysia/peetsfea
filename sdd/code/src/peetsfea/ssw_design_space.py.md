...
- Responsibility: parse 0.3.0 SSW sweep-style TOML range surfaces, check candidate inclusion against the reference sweep space, build short AEDT point identities, and sample deterministic fixed TOML points.
...
- Outputs: typed design-space check results, `SswAedtIdentity`, and sampled TOML batch metadata with generated paths and point identities.
- Canonical state: role-qualified free owner paths from the reference sweep, realized candidate point values used in canonical JSON hash payloads, and sampled fixed TOML files named by point identity.
...
- Hazards: do not include seed, TOML comments, TOML ordering, grid index, or candidate count in the point hash; do not let the identity stem affect AEDT imported object/body names; keep sampling deterministic without mutating the source sweep TOML.
