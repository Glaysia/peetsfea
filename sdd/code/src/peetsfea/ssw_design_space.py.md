...
- Responsibility: parse 0.3.1 SSW sweep-style TOML range surfaces, check candidate inclusion against the reference sweep space, build short AEDT point identities, and sample deterministic fixed TOML points with explicit realized point values that pass the SSW scene geometry contract.
...
- Invariants: free paths are reference ranges with `count != 1`, modeled object paths include `role=...`, candidate ranges must be present for every free path, candidate count must be a positive integer, candidate bounds must stay inside reference bounds, TX/RX `void_profile` are fixed point ranges and do not enter the 0.3.1 free-owner set, non-reference varying ranges are rejected for sampling, sampled fixed TOMLs must parse and pass `build_ssw_body_boxes` before acceptance, and AEDT identity generation requires every free path to be a single realized value.
