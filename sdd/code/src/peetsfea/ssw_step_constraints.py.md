...
- Invariants: every rule has `id`, `kind`, `message`, `enabled`, `lhs`, `op`, and `rhs`; supported operands are exactly `{path=...}`, `{value=...}`, and `{func=...}`; supported function calls are limited to `ssw_conductor_component_count(object_id)`; SSW-enabled coils must evaluate to one conductor component to pass the public example constraints.
