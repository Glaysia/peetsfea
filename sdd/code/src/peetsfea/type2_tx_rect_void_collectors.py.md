...
- Build underside parallel collector pour geometry for `tx_rect_void_columns` after tile tilt and vertical terminal-stub generation, producing a balanced two-tab future port handoff for realized `connection_mode = 0`.
...
- Outputs: deterministic start/end pour bus and branch-patch source solids, end layer-drop solids, one fused copper handoff body, grouped source labels, two external terminal-tab face vertex sets, branch balance audit data, and overlap audit data.
...
- Only `connection_mode = 0` may call this module.
...
- Do not route series or hybrid series-parallel collectors through this module.
