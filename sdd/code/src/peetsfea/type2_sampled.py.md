...
- `tx_rect_void_columns` no longer has inactive mode-dependent turn owners; sampled metadata includes the single active `equivalent_turn_count` owner when its source range has more than one candidate.
...
- tx-columns equivalent-turn owner filtering must keep the realized-grid and connection-mode feasibility filter; removing it can make ordinary sample+STEP runs fail for valid TOML ranges.
