...
- Canonical state: fixed dimensions including the TV-to-TX spacing, TX/RX coil parameters, `tv` and derived `tx_region` non-model bounds/transparency, SSW port-anchor world coordinates, scene action tokens, generated part names, body bounds, and body roles in the ledger.
- Invariants: TX and RX modeled objects must exist once, all ranges in fixed mode have `min == max` and `count == 1`, dimensions are positive, RX is a YZ-plane SSW coil inside the TV with FR4 aligned to the TV bottom and copper overhang limited to copper thickness, RX's SSW port anchor sits on the TV back-facing `x_min` copper face, TX is an XY-plane SSW coil inside the derived `tx_region` with its port anchor on the lower `z_min` face, TX/RX long dimensions follow the Y axis, `tx_region` is larger than the TX coil and its top equals `TV bottom - fixed_dimensions.tx_rx_min_distance_mm`, token TOML is saved before STEP export, body names are unique, and CadQuery STEP export must produce a non-empty file.
...
- Change hazards: keep SSW-specific fields separate from deprecated normal-coil and serial-coil fields; do not rotate TX into the TV plane, move the TX port anchor off the lower face, move the RX port anchor off the TV back face, collapse `tx_region` into a coil-tight box, move RX out of the TV bottom region, or add fallback minimal two-port geometry.
