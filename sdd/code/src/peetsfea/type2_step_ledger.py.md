...
- `tx_outer_actual_region` member typing is reserved for a real outer TX modeled source and must not be populated from guide-only placeholders.
...
- `tx_outer_actual_region` must be absent until a concrete outer TX modeled source exists.
