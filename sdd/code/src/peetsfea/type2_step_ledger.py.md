...
- TX underlay ownership은 별도 ledger subtree가 아니라 modeled metadata의 exact `expected_exported_body_names` / `expected_exported_body_count` 계약에 포함된다.
...
- `underlay_repeat_count`를 ledger-owned duplicated runtime state로 만들지 않는다. exact body names/count contract 하나만 canonical handoff로 유지한다.
