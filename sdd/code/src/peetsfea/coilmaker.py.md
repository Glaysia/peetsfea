...
- Invariants: SSW generation is trace-first, SSW-enabled `TURN_N_INT` and `TWIST_FACTOR` must be coprime so generated copper is one intended conductor, normal/spiral port landing pad size follows the realized trace width, normal/spiral landing uses the pad edge perpendicular to the outer-terminal trace segment's cardinal direction, action token payloads stay scalar/tuple/ref based, and generated assemblies are deterministic for the same config.
- Fail-fast points: invalid dimensions, non-coprime SSW turn/twist combinations, collapsed trace geometry, unsupported SSW trace construction, non-transformer-ready token payloads, empty rendered parts, and failed export-oriented generation.
