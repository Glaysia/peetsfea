...
- Responsibility: generate the CadQuery SSW coil assembly and transformer-ready action trace from validated coil dimensions and SSW parameters.
- Inputs: `RuntimeConfig` with fixed dimensions, common coil parameters, spiral parameters, and SSW parameters.
- Outputs: CadQuery assembly children for FR4 boards and SSW copper, plus materialized action tokens that can be serialized as TOML by callers.
- Canonical state: dataclass parameter values, derived frame dimensions, action token refs, and generated CadQuery child names.
- Invariants: SSW generation is trace-first, action token payloads stay scalar/tuple/ref based, generated copper is one intended conductor, and generated assemblies are deterministic for the same config.
...
- Change hazards: do not reintroduce normal-coil fallback or minimal two-port placeholder behavior for the 0.3.0 SSW path.
