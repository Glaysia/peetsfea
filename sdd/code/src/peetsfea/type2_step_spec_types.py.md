...
- `NonModelTxRegionSpec` extends the regular box spec with required `z_gap_from_rx_plane_mm` and TX reference-line state while preserving box fields used by downstream guide paths.
...
- `NonModelTxRegionSpec.z_gap_from_rx_plane_mm` must be a non-integer `RangeSpec` whose realized candidates are all strictly positive.
