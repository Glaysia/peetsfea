...
- `non_model_objects.tx_region.tx_reference_line.x_ratio` remains fixed at `[false, 0.99, 0.99, 1]`; with `tx_region.size_xyz[0] = 720.0`, it gives the fixed `tx_inner_region` a 712.8 mm X span.
...
- `modeled_objects.tx_inner_rect_void_coil.outer_x_usage_ratio` and `outer_y_usage_ratio` are fixed to map the active `tx_inner_region` owner span to the selected 0.2.21 TX DD outer size.
...
- `modeled_objects.tx_inner_rect_void_coil.turn_count` remains fixed at two turns for the fixed example.
...
- `modeled_objects.tx_inner_rect_void_coil.void_stack_present` remains fixed disabled at `[true, 0, 0, 1]`; the selected 0.2.21 topology does not carry the active type2 TX inner void-stack concept and the small two-turn TX geometry does not satisfy the void-stack corridor proof.
- `modeled_objects.rx_rect_void_coil` remains active with role `rx_single_coil`.
- `modeled_objects.rx_rect_void_coil.outer_x_usage_ratio` and `outer_y_usage_ratio` are fixed to map the active RX owner span to the selected 0.2.21 RX DD outer size.
- `modeled_objects.rx_rect_void_coil.turn_count` remains fixed at three turns for the fixed example.
