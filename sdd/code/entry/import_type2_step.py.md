...
- planned future contract에서는 imported ledger 직후 runtime-owned post-import mesh 단계까지 포함한 setup-ready handoff entrypoint로 확장된다.
...
- future setup-ready baseline에서는 importer가 imported ledger 작성 직후 `MeshSetup.AssignLengthOp(...)` exact contract를 적용한다: `Length1`, objects `["tx_copper_l0", "rx_copper_l0"]`, `RefineInside=False`, `Enabled=True`, `RestrictElem=False`, `NumMaxElem="1000"`, `RestrictLength=True`, `MaxLength="5mm"`.
...
- [ ] `.aedt`와 imported/setup ledger를 남긴 뒤, exact post-import mesh contract를 포함한 runtime/entry handoff를 정의한다. notebook은 thin manual consumer로 유지한다.
