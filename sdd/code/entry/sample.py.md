...
- CLI accepts `--seed-first`, `--seed-n`, and `--sampler-n` for bounded diagnostic/operator runs while preserving module defaults when omitted.
...
- CLI `--seed-first` must be zero or greater, and CLI `--seed-n` / `--sampler-n` must be one or greater.
- Seed/worker CLI overrides must validate explicitly and must not change module constants.
