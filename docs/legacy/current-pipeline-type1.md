...
```mermaid
flowchart TD
    Spec["run/legacy/type1.toml 또는 frozen resolved TOML"]

    SamplePy["entry/legacy/type1/sample.py"]
    ...
    BuildPy["entry/legacy/type1/build.py"]
    SampleBuild["entry/legacy/type1/sample_build.py"]
...
## 엔트리포인트 역할
- `entry/legacy/type1/sample.py`: 기본 batch profile 집합을 계산하고, 각 batch마다 resolved TOML들과 batch `manifest.json`을 생성한다.
- `entry/legacy/type1/build.py`: `entry/legacy/type1/sample.py`와 같은 batch profile 집합을 다시 순회하면서 각 `manifest.json`을 읽어 build를 수행한다.
- `entry/legacy/type1/sample_build.py`: 새 sample을 만들지 않고, 이미 만들어진 batch `manifest.json`들을 순차 replay build하는 디버그 entry다.

현재 entry 계층에서 사라진 것:
- `multi_sample.py`, `multi_build.py`, `build_one.py`, `sample_one_build.py` 같은 별도 우회 entry는 현재 active path가 아니다.

실행 특성:
- `entry/legacy/type1/sample.py`는 batch 간 병렬화는 가능하지만, batch 내부 design 생성은 현재 직렬로 수행한다.
- `entry/legacy/type1/build.py`는 headless runtime일 때만 manifest 내부 entry build를 병렬화할 수 있다.
- GUI-visible build는 항상 순차 실행된다.
- `stop_on_error=False`는 더 이상 지원되지 않는다.

## 샘플링과 selection 단계
- 샘플링 시작점은 `entry/legacy/type1/sample.py::generate_all_sample_manifests()`다.
...
- build 시작점은 `entry/legacy/type1/build.py::build_all_targets_with_options()`다.
...
| --- | --- | --- | --- |
| 원본 입력 TOML | 샘플 시작 전부터 존재 | 보통 `run/legacy/type1.toml` | sampling space의 원본 SSOT다. |
...
현재 VS Code 디버그 경로:
- `legacy-type1-run-sample-debug` task는 `run/toml/`을 비운 뒤 `run/`에서 `entry/legacy/type1/sample.py`를 실행한다.
- `legacy-type1 Run entry/sample_build.py from run/` launch는 `prepare-build-debug` 이후 `run/`에서 `entry/legacy/type1/sample_build.py`를 실행한다.
- `entry/legacy/type1/sample_build.py`는 기존 manifest replay만 수행하며, 새 sample 생성은 하지 않는다.
