# Runner Integration (peetsfea 0.3.5)
...
```python
peetsfea.__version__                       # "0.3.5"; package ships py.typed
...
peetsfea.run_ssw_random_sample_reports_from_toml_text(
    candidate_toml_text: str, *, output_dir, seed, mode,
...
## Contract (all implemented in 0.3.5)

| # | Requirement | Where |
|---|-------------|-------|
| 1 | `__version__ == "0.3.5"` + `py.typed` for strict typing of the public API | `version.py`, `py.typed` |
...
| 7 | Structured result `SswRandomSampleReportResult` (TypedDict): output vars + input params via report CSVs, per-setup pass counts, solve telemetry, `output_dir`/`aedt_path` | `ssw_random_sample_reports.py` |
...
The reference sweep design space is bundled **inside the package** as `peetsfea/data/0.3.5_sweep.toml`
...
`peetsfea/data/0.3.5_fixed.toml` (`DEFAULT_SOURCE_TOML_PATH`).
...
- **Reference SSOT**: `peetsfea/data/0.3.5_sweep.toml`, resolved install-safe.
