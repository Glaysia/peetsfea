...
- 기준 릴리즈는 현재 문서 기준과 같은 `0.2.17`이다.
...
| `src/peetsfea/spec/resolver/group_geometry.py` | `turn_count_max`, `band_ratio`, `metal_ratio`에서 `trace`/`gap`을 파생한다. | 조건부 재사용 | 파생 공식은 공통 후보가 될 수 있지만 현재 `GROUP_KIND_ORDER`와 `tx_vertical` 특례를 안고 있다. | kind별 외곽 치수 조회를 인터페이스로 빼고 `type2` geometry family를 추가한다. |
...
| `entry/sample.py`, `entry/multi_sample.py`, `entry/build.py`, `entry/multi_build.py` | 현재 표준 배치 샘플/빌드 진입점을 제공한다. | 그대로 재사용 | entry 스크립트는 `run()`과 `run_batch` orchestration 위에 얹힌 thin wrapper다. | 기본 TOML 경로와 geometry dispatch만 `type2` 선택을 허용하면 된다. |
| `entry/build_one.py`, `entry/sample_one_build.py` | GUI-visible build와 단건 sample+build 디버그 플로우를 제공한다. | 조건부 재사용 | 디버그 워크플로는 공통이지만 현재 기본 profile과 source TOML이 `type1` 전용이다. | `type2` debug profile과 source TOML 선택 방식을 분리한다. |
...
probe = _probe_cad_object(obj, fallback_name)
name = safe_unite(modeler=modeler, targets=targets, fallback_name=name, error_context=context)
```
...
| `src/peetsfea/placement_math.py::tx_vertical_mode2_center_x_from_tx_dd_min()` | ratio 기반 위치 계산을 수행한다. | 조건부 재사용 | 수학 함수 자체는 단순하지만 현재 의미가 `tx_vertical mode2`에 직접 묶여 있다. | 이름과 입력 의미를 일반화한 placement utility로 승격한다. |
| `src/peetsfea/backend/pyaedt/geometry/cad_probe.py` | live CAD object의 bbox/edge sample을 추출한다. | 그대로 재사용 | 검증/메타데이터용 probe는 coil 종류와 무관하다. | 새 객체 이름 규칙을 써도 `fallback_name` 기준으로 계속 사용한다. |
