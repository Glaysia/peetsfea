...
- `coil_placement`: placement contracts. The default example samples `coil_placement.neo_tx_dd_top_offset_ratio` over `0.0..0.3` with `count=10`; it is the downward top-clearance ratio relative to `tx.region.z_parts.dd_z_mm`. `coil_placement.tx_vertical_orientation_mode` means `1=ZX`, and the current runnable example is fixed to `1 (ZX)`. The default example keeps `coil_groups.tx_vertical.count_range` sampled over `1..6` for legacy ZX mode. The public spec no longer accepts `coil_placement.tx_vertical_plane`; only the realized plane remains as an internal derived field. Common geometry keys have been moved to `coil_shape.neo_tx_vertical.*` and `coil_groups_params.neo_tx_vertical.*` for the neo migration, while vertical-specific placement keys still keep the `tx_vertical_*` naming for now. `coil_placement.neo_tx_dd_right_terminal_path` and `coil_placement.neo_tx_dd_left_terminal_path` define the terminal-path contract for the right and left neo TX DD coils, and `coil_placement.neo_tx_vertical_zx_terminal_path` defines the neo TX vertical terminal-path contract for ZX/XZ coils. All values use the `<start>_<cw|ccw>_to_<end>` format, and the default example uses `B_ccw_to_c`.
