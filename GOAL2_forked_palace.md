...
- sanity: ferrite-enabled(§3.1) 대비 **L↓·k↓** 방향 맞는지(ferrite 자속경로 제거 효과).

## Acceptance
- [ ] HFSS에서 TX ferrite non-model로 6.78 MHz solve 완주, report 추출.
- [ ] §3.2의 Z/L/M/k/R/Q가 실제 HFSS no-ferrite 실측으로 채워짐(— TBD 제거).
- [ ] ferrite-enabled 대비 L↓·k↓ sanity 통과.
- [ ] 증거: report CSV + 로그 + ferrite non-model 확인.
