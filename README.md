...
## 현재 계약
- 버전: `0.3.5`
- 설계 공간 SSOT: 패키지 데이터 `src/peetsfea/data/0.3.5_sweep.toml` (`DEFAULT_REFERENCE_TOML_PATH`). 정규 fixed 점은 `src/peetsfea/data/0.3.5_fixed.toml` (`DEFAULT_SOURCE_TOML_PATH`). 둘 다 wheel에 동봉되어 설치 환경에서도 해석됩니다.
...
- MULL ferrite 위치는 TX Z축 `ferrite.tx_mull_position_ratio`와 RX X축 `ferrite.rx_mull_position_ratio`로 따로 제어합니다.
...
# 정규 fixed 점(data/0.3.5_fixed.toml)을 생성·표시
...
- `peetsfea.__version__ == "0.3.5"`, 패키지에 `py.typed` 동봉.
