...
from peetsfea.backend.pyaedt.type2_step_import_style import style_imported_modeled_objects
...
_TX_INNER_SINGLE_COIL_FERRITE_GROUP_MEMBER_PREFIXES: tuple[str, ...] = (
    "tx_underlay_pet_psa_u",
    "tx_underlay_ferrite_u",
    "tx_void_ferrite_u",
    "tx_void_pet_psa_u",
)
_TX_OUTER_SINGLE_COIL_FERRITE_GROUP_MEMBER_PREFIXES: tuple[str, ...] = (
...
def _tx_outer_single_coil_entry(
    *,
    ...
    outer_tilt_z_underhang_mm: float = 0.0,
) -> dict[str, object]:
    ...
    if outer_tilt_protrusion_mm is not None:
        canonical_coordinates["outer_tilt_metadata"] = {
            "max_world_x_protrusion_mm": outer_tilt_protrusion_mm,
            "max_world_z_underhang_mm": outer_tilt_z_underhang_mm,
        }
...
def _non_model_entry_with_tx_inner_region(
    *,
    inner_region_origin_xyz: tuple[float, float, float] = (0.0, -40.0, 40.0),
    inner_region_size_xyz: tuple[float, float, float] = (56.0, 80.0, 30.0),
    actual_region_origin_xyz: tuple[float, float, float] = (0.0, -40.0, 40.0),
    actual_region_size_xyz: tuple[float, float, float] = (56.0, 80.0, 30.0),
    physical_modeled_body_origin_xyz: tuple[float, float, float] = (3.0, -15.0, 67.2),
    physical_modeled_body_size_xyz: tuple[float, float, float] = (50.0, 30.0, 2.8),
) -> dict[str, object]:
    non_model_object = _non_model_entry()
    inner_region_origin_x, inner_region_origin_y, inner_region_origin_z = inner_region_origin_xyz
    inner_region_size_x, inner_region_size_y, inner_region_size_z = inner_region_size_xyz
    actual_region_origin_x, actual_region_origin_y, actual_region_origin_z = actual_region_origin_xyz
    actual_region_size_x, actual_region_size_y, actual_region_size_z = actual_region_size_xyz
    physical_modeled_body_origin_x, physical_modeled_body_origin_y, physical_modeled_body_origin_z = (
        physical_modeled_body_origin_xyz
    )
    physical_modeled_body_size_x, physical_modeled_body_size_y, physical_modeled_body_size_z = (
        physical_modeled_body_size_xyz
    )
    inner_region_member = {
        "object_id": "tx_inner_region",
        "role": "tx_inner_region",
        "material": "vacuum",
        "model_state": False,
        "canonical_coordinates": {
            "frame_origin_xyz": list(inner_region_origin_xyz),
            "outer_bounds_min_xyz": list(inner_region_origin_xyz),
            "outer_bounds_max_xyz": [
                inner_region_origin_x + inner_region_size_x,
                inner_region_origin_y + inner_region_size_y,
                inner_region_origin_z + inner_region_size_z,
            ],
            "outer_bounds_size_xyz": list(inner_region_size_xyz),
        },
        "plane": "XY",
        "non_model": True,
        "member_object_ids": ("tx_inner_region",),
    }
    actual_region_bounds = {
        "min_xyz": list(actual_region_origin_xyz),
        "max_xyz": [
            actual_region_origin_x + actual_region_size_x,
            actual_region_origin_y + actual_region_size_y,
            actual_region_origin_z + actual_region_size_z,
        ],
        "size_xyz": list(actual_region_size_xyz),
    }
    physical_modeled_body_bounds = {
        "min_xyz": list(physical_modeled_body_origin_xyz),
        "max_xyz": [
            physical_modeled_body_origin_x + physical_modeled_body_size_x,
            physical_modeled_body_origin_y + physical_modeled_body_size_y,
            physical_modeled_body_origin_z + physical_modeled_body_size_z,
        ],
        "size_xyz": list(physical_modeled_body_size_xyz),
    }
    actual_region_member = {
        "object_id": "tx_inner_actual_region",
        "role": "tx_inner_actual_region",
        "material": "vacuum",
        "model_state": False,
        "canonical_coordinates": {
            "frame_origin_xyz": list(actual_region_origin_xyz),
            "outer_bounds_min_xyz": list(actual_region_origin_xyz),
            "outer_bounds_max_xyz": actual_region_bounds["max_xyz"],
            "outer_bounds_size_xyz": list(actual_region_size_xyz),
        },
        "plane": "XY",
        "non_model": True,
        "member_object_ids": ("tx_inner_actual_region",),
        "tx_actual_region": {
            "source_guide_id": "tx_inner_region",
            "modeled_source_id": "tx_inner_rect_void_coil",
            "x_usage_ratio_owner_path": "modeled_objects.tx_inner_rect_void_coil.outer_x_usage_ratio",
            "y_usage_ratio_owner_path": "modeled_objects.tx_inner_rect_void_coil.outer_y_usage_ratio",
            "x_usage_ratio": 1.0,
            "y_usage_ratio": 1.0,
            "guide_bounds": {
                "min_xyz": list(inner_region_origin_xyz),
                "max_xyz": [
                    inner_region_origin_x + inner_region_size_x,
                    inner_region_origin_y + inner_region_size_y,
                    inner_region_origin_z + inner_region_size_z,
                ],
                "size_xyz": list(inner_region_size_xyz),
            },
            "actual_region_bounds": actual_region_bounds,
            "physical_modeled_body_bounds": physical_modeled_body_bounds,
        },
    }
    member_object_ids = list(cast(tuple[str, ...], non_model_object["member_object_ids"]))
    member_object_ids.append("tx_inner_region")
    member_object_ids.append("tx_inner_actual_region")
    non_model_object["member_object_ids"] = tuple(member_object_ids)
    member_objects = list(cast(list[dict[str, object]], non_model_object["member_objects"]))
    member_objects.append(inner_region_member)
    member_objects.append(actual_region_member)
    non_model_object["member_objects"] = member_objects
    return non_model_object


def _non_model_entry_with_tx_bridge_members() -> dict[str, object]:
    non_model_object = _non_model_entry()
    tx_bridge_member_ids = ("tx_pos_bridge_pcb", "tx_pos_bridge_copper", "tx_neg_bridge_pcb", "tx_neg_bridge_copper")
    member_object_ids = list(cast(tuple[str, ...], non_model_object["member_object_ids"]))
    for member_id in tx_bridge_member_ids:
...
def _tx_inner_underlay_expected_names(*, repeat_count: int) -> list[str]:
    if repeat_count == 0:
        return []
    return [
...
def _tx_inner_void_stack_expected_names(*, repeat_count: int) -> list[str]:
    if repeat_count == 0:
        return []
    return [
        "tx_void_ferrite_u0",
...
def _tx_inner_modeled_object_with_role_aware_underlay(
    tmp_path: Path,
    *,
    tx_underlay_repeat_count: int = 0,
    tx_void_stack_repeat_count: int = 0,
) -> dict[str, object]:
    expected_names = [
        "tx_inner_pcb_l0",
        "tx_inner_copper_l0",
        *_tx_inner_underlay_expected_names(repeat_count=tx_underlay_repeat_count),
        *_tx_inner_void_stack_expected_names(repeat_count=tx_void_stack_repeat_count),
    ]
    return _modeled_entry(
...
def _tx_inner_imported_name_batch_with_role_aware_underlay(
    *,
    tx_underlay_repeat_count: int = 0,
    tx_void_stack_repeat_count: int = 0,
) -> tuple[str, ...]:
...
def _expected_ferrite_group_for_role(*, role: str, expected_names: list[str]) -> list[ExportedBodyGroup]:
    ...
    if len(member_body_names) == 0:
        return []
    if role == "tx_outer_single_coil":
        return [{"group_name": _TX_OUTER_FERRITE_GROUP_NAME, "member_body_names": member_body_names}]
    if role.startswith("tx_"):
...
def test_resolve_imported_body_groups_accepts_tx_inner_single_coil_with_actual_underlay_group_contract() -> None:
...
def test_load_step_ledger_accepts_tx_inner_single_coil_with_void_stack_group_contract(tmp_path: Path) -> None:
    ...
    _write_ledger(
        ...
        modeled_objects=[tx_inner_object, _rx_single_coil_entry(tmp_path)],
    )

    ledger = load_step_ledger(ledger_path)
...
def test_style_imported_modeled_objects_materializes_tx_inner_single_coil_void_stack_passively(tmp_path: Path) -> None:
    modeled_entry = _tx_inner_modeled_object_with_role_aware_underlay(
        tmp_path,
        tx_underlay_repeat_count=1,
        tx_void_stack_repeat_count=1,
    )
    ...
    modeler = _FakeModeler(imported_name_batches=[])
...
def test_style_imported_modeled_objects_rejects_tx_outer_single_coil_void_stack(tmp_path: Path) -> None:
    modeled_entry = _modeled_entry(
        ...
        expected_groups=_expected_ferrite_group_for_role(
            role="tx_outer_single_coil",
            expected_names=[
                "tx_outer_pcb_l0",
                "tx_outer_copper_l0",
        ...
        source_metadata_path=str(tmp_path / "tx_outer.metadata.json"),
    )
    imported_object_names = [
        "tx_outer_pcb_l0",
        "tx_outer_copper_l0",
...
def test_import_type2_step_ledger_rejects_tx_outer_void_stack_at_tx_region_top(tmp_path: Path) -> None:
    ...
    modeled_entry = _tx_outer_single_coil_entry(
        origin_xyz=(0.0, -130.0, 0.0),
        size_xyz=(80.0, 260.0, 90.0),
        outer_tilt_protrusion_mm=0.0,
    )
    modeled_entry["expected_exported_body_names"] = expected_names
    modeled_entry["expected_exported_body_count"] = len(expected_names)
    modeled_entry["expected_exported_body_groups"] = _expected_ferrite_group_for_role(
        role="tx_outer_single_coil",
        expected_names=expected_names,
    )
    _write_ledger(
...
def test_import_type2_step_ledger_keeps_tx_bridge_members_in_non_model_scene(tmp_path: Path) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    tx_bridge_member_ids = ("tx_pos_bridge_pcb", "tx_pos_bridge_copper", "tx_neg_bridge_pcb", "tx_neg_bridge_copper")
    _write_ledger(
        ledger_path,
        scene_step_path=scene_step,
        non_model_objects=[_non_model_entry_with_tx_bridge_members()],
        modeled_objects=_single_layer_modeled_objects(tmp_path),
    )
    imported_names = _single_layer_imported_name_batch() + tx_bridge_member_ids
    session = _FakeHfss(modeler=_FakeModeler(imported_name_batches=[imported_names]))
    ...
    modeled_by_id = {cast(str, entry["object_id"]): entry for entry in result["modeled_objects"]}
    non_model_by_id = {cast(str, entry["object_id"]): entry for entry in result["non_model_objects"]}
    assert modeled_by_id["tx_rect_void_coil"]["imported_object_names"] == ["tx_pcb_l0", "tx_copper_l0", "tx_port_sheet"]
    assert modeled_by_id["rx_rect_void_coil"]["imported_object_names"] == ["rx_pcb_l0", "rx_copper_l0", "rx_port_sheet"]
    ...
    tx_imported_names = cast(list[str], modeled_by_id["tx_rect_void_coil"]["imported_object_names"])
    assert non_model_imported_names[-4:] == list(tx_bridge_member_ids)
    for member_id in tx_bridge_member_ids:
        assert member_id not in tx_imported_names
...
def test_import_type2_step_ledger_styles_tx_inner_single_coil_with_underlay_group_contract(tmp_path: Path) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    tx_inner_object = _tx_inner_modeled_object_with_role_aware_underlay(
        tmp_path,
        tx_underlay_repeat_count=1,
    ...
    session = _FakeHfss(
        modeler=_FakeModeler(
            imported_name_batches=[_tx_inner_imported_name_batch_with_role_aware_underlay(tx_underlay_repeat_count=1)]
        )
    )
    session.materials.delayed_lookup_material_names.add("PET_PSA")
    ...
    _write_ledger(
        ...
        modeled_objects=[
            _tx_outer_single_coil_entry(
                ...
                outer_tilt_z_underhang_mm=0.5,
                origin_xyz=(0.0, -140.0, -0.25),
...
def test_import_type2_step_ledger_rejects_tx_outer_single_coil_invalid_outer_tilt_metadata(
    ...
    canonical_coordinates["outer_tilt_metadata"] = {
        "max_world_x_protrusion_mm": -5.0,
        "max_world_z_underhang_mm": 0.0,
    }
    _write_ledger(
        ledger_path,
        scene_step_path=scene_step,
...
def test_import_type2_step_ledger_accepts_tx_inner_single_coil_sampled_x_inside_tx_inner_region(
    tmp_path: Path,
) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    modeled_origin_xyz = (3.4939466292134824, -15.0, 67.2)
    modeled_size_xyz = (40.47134831460676, 30.0, 2.8)
    _write_ledger(
        ledger_path,
        scene_step_path=scene_step,
        non_model_objects=[
            _non_model_entry_with_tx_inner_region(
                physical_modeled_body_origin_xyz=modeled_origin_xyz,
                physical_modeled_body_size_xyz=modeled_size_xyz,
            )
        ],
        modeled_objects=[
            _modeled_entry(
                ...
                origin_xyz=modeled_origin_xyz,
                size_xyz=modeled_size_xyz,
                expected_groups=[],
            )
        ],
    )
    ...
    modeled_by_id = {entry["object_id"]: entry for entry in result["modeled_objects"]}
    modeled_entry = cast(dict[str, object], modeled_by_id["tx_inner_rect_void_coil"])
    assert modeled_entry["placement_owner_id"] == "tx_inner_region"
    assert modeled_entry["role"] == "tx_inner_single_coil"


def test_import_type2_step_ledger_accepts_tx_inner_actual_region_when_physical_bounds_are_smaller_asymmetric(
    tmp_path: Path,
) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    modeled_origin_xyz = (6.538651685393255, -84.0, 39.56499999999998)
    modeled_size_xyz = (40.47134831460676, 164.07134831460672, 30.935000000000002)
    _write_ledger(
        ledger_path,
        scene_step_path=scene_step,
        non_model_objects=[
            _non_model_entry_with_tx_inner_region(
                inner_region_origin_xyz=(0.0, -140.0, 0.0),
                inner_region_size_xyz=(56.00000000000001, 280.0, 70.49999999999999),
                actual_region_origin_xyz=(2.609999999999996, -84.0, 39.56499999999998),
                actual_region_size_xyz=(44.400000000000006, 168.0, 30.935000000000002),
                physical_modeled_body_origin_xyz=modeled_origin_xyz,
                physical_modeled_body_size_xyz=modeled_size_xyz,
            )
        ],
...
def test_import_type2_step_ledger_rejects_tx_inner_actual_region_bad_centering_provenance(
    tmp_path: Path,
) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    modeled_origin_xyz = (6.538651685393255, -84.0, 39.56499999999998)
    ...
    _write_ledger(
        ...
        non_model_objects=[
            _non_model_entry_with_tx_inner_region(
                inner_region_origin_xyz=(0.0, -140.0, 0.0),
                inner_region_size_xyz=(56.00000000000001, 280.0, 70.49999999999999),
                actual_region_origin_xyz=(2.609999999999996, -83.0, 39.56499999999998),
                actual_region_size_xyz=(44.400000000000006, 168.0, 30.935000000000002),
                physical_modeled_body_origin_xyz=modeled_origin_xyz,
                physical_modeled_body_size_xyz=modeled_size_xyz,
            )
    ...
    with pytest.raises(ValueError, match=r"tx_inner_actual_region"):
        import_type2_step_ledger(
            step_ledger_path=ledger_path,
            output_aedt_path=tmp_path / "aedt" / "type2_import.aedt",
            imported_ledger_path=tmp_path / "aedt" / "type2_imported_ledger.json",
...
def test_import_type2_step_ledger_rejects_tx_inner_single_coil_sampled_x_outside_tx_inner_region(
    tmp_path: Path,
) -> None:
    scene_step, ledger_path = _source_paths(tmp_path)
    modeled_origin_xyz = (20.0, -15.0, 67.2)
    modeled_size_xyz = (40.47134831460676, 30.0, 2.8)
    _write_ledger(
        ledger_path,
        scene_step_path=scene_step,
    ...
    with pytest.raises(
        ValueError,
        match=r"physical modeled bounds must stay contained in tx_inner_actual_region design bounds",
    ):
        import_type2_step_ledger(
            step_ledger_path=ledger_path,
            output_aedt_path=tmp_path / "aedt" / "type2_import.aedt",
