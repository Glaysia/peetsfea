...
class _FakeAnalyzeHfss:
    ...
    def __init__(self, *, analyze_sleep_seconds: float = 0.0, analyze_returns: bool = True) -> None:
...
def test_hard_abort_completes_within_budget() -> None:
    ...
    outcome = _analyze_with_hard_abort(hfss=cast(HfssSession, raw), setup_name="Setup1", hard_abort_seconds=10.0)
    assert outcome["completed"] is True
    assert outcome["hard_aborted"] is False
    assert outcome["hard_abort_seconds"] == 10.0
...
def test_hard_abort_triggers_clean_stop_and_reports_abort() -> None:
    ...
    outcome = _analyze_with_hard_abort(
...
def test_analyze_failure_raises_structured_solve_error() -> None:
    ...
    with pytest.raises(PeetsfeaStageError) as excinfo:
        _analyze_with_hard_abort(hfss=cast(HfssSession, raw), setup_name="Setup1", hard_abort_seconds=10.0)
