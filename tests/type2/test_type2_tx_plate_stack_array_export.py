...
def _single_solid_volume(*, shape: Shape, label: str) -> float:
    solids = tuple(shape.solids())
    if len(solids) != 1:
        raise AssertionError(f"{label} expected one solid, actual={len(solids)}")
    volume = solids[0].volume
    if volume <= 0.0:
        raise AssertionError(f"{label} expected positive volume, actual={volume}")
    return volume


def _intersection_volume(*, first_shape: Shape, second_shape: Shape) -> float:
    raw_intersection_shape_or_list = first_shape.intersect(second_shape)
    assert isinstance(raw_intersection_shape_or_list, (Shape, ShapeList))
    intersection_shape_or_list: Shape | ShapeList[Shape] = raw_intersection_shape_or_list
    if isinstance(intersection_shape_or_list, ShapeList):
        return sum(solid.volume for shape in intersection_shape_or_list for solid in shape.solids())
    return sum(solid.volume for solid in intersection_shape_or_list.solids())
...
def test_export_type2_tx_plate_stack_array_contract(tmp_path: Path) -> None:
    ...
    assert tx_groups[1]["member_body_names"] == tuple(
        member_name
    ...
    for branch_index in range(3):
        ...
        source_branch_shapes_by_label = {
            branch_shape.label: branch_shape
            for branch_shape in branch_shapes
            if branch_shape.label in ("tx_pcb_wall", "tx_pcb_coil")
        }
        assert set(source_branch_shapes_by_label) == {"tx_pcb_wall", "tx_pcb_coil"}
        branch_ferrite_shapes = tuple(
            scene_shapes_by_label[f"tx_b{branch_index}_{suffix}"]
            for suffix in ("stack_pet_psa", "stack_ferrite", "stack_air")
        )
        for suffix, source_label in (("pcb_wall", "tx_pcb_wall"), ("pcb_coil", "tx_pcb_coil")):
            cut_pcb_shape = scene_shapes_by_label[f"tx_b{branch_index}_{suffix}"]
            cut_pcb_volume = _single_solid_volume(
                shape=cut_pcb_shape,
                label=f"tx_b{branch_index}_{suffix}",
            )
            source_pcb_volume = _single_solid_volume(
                shape=source_branch_shapes_by_label[source_label],
                label=source_label,
            )
            assert cut_pcb_volume <= source_pcb_volume + 1e-6
            assert all(
                _intersection_volume(first_shape=cut_pcb_shape, second_shape=ferrite_shape) <= 1e-6
                for ferrite_shape in branch_ferrite_shapes
            )
