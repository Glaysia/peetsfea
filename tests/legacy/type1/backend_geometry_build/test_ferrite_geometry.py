...
from peetsfea.legacy.type1.backend.pyaedt.geometry.builders.build_artifacts import build_em_artifacts
from peetsfea.legacy.type1.backend.pyaedt.geometry.rules.scene_objects import (
...
from peetsfea.types.manifest import CadProbe, GroupObjects, SceneObjectEntry, SelectedParameters, SelectedParametersMax
...
class _FakeModeler:
    ...
    def create_box(self, **kwargs: object) -> _FakeObject:
        ...
        obj = _FakeObject(name=name, origin=origin, sizes=sizes)
    ...
    def get_object_faces(self, assignment: str) -> list[int]:
        assert assignment in self.objects or assignment.startswith(("coil_", "bridge_", "fr4_", "neo_", "scene_", "rxs_", "rxc_", "txs_"))
        return [1]

    def set_object_model_state(self, object_name: str, is_model: bool) -> bool:
        _ = object_name, is_model
        return True


class _FalseCreateBoxModeler(_FakeModeler):
    def create_box(self, **kwargs: object) -> bool:
        self.box_calls.append(dict(kwargs))
        return False


def _selected(*, ferrite_present: bool, tx_vertical_orientation_mode: int = 1) -> SelectedParameters:
    return cast(
        SelectedParameters,
        {
            "tx_dd_outer_x": 140.0,
...
def _selected_max() -> SelectedParametersMax:
    return cast(
...
def _tx_live_object_names() -> list[str]:
    return ["bridge_tx_demo", "coil_tx_high_demo", "coil_tx_low_demo", "neo_fr4_tx_dd_tx_main_0_l0_demo"]


def _rx_live_object_names() -> list[str]:
    return ["bridge_rx_dd_d_to_b_demo", "coil_rx_demo", "fr4_rx_shared_yz_0_demo"]


def test_create_split_ferrite_model_objects_create_expected_plates_and_subtract_targeted_live_tools() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()

    tx_names, tx_probes, tx_entries = _create_tx_ferrite_model_objects(
        ...
        live_object_names=_tx_live_object_names(),
    )
    rx_names, rx_probes, rx_entries = _create_rx_ferrite_model_objects(
        modeler=cast(Modeler3D, modeler),
        hfss=cast(Hfss, hfss),
        design_id="demo",
        selected=_selected(ferrite_present=True),
        ...
        tx_board_ids={"tx_main_0", "tx_main_1"},
        live_object_names=_rx_live_object_names(),
    )

    names = tx_names + rx_names
    probes = tx_probes + rx_probes
    entries = tx_entries + rx_entries
    assert names == ["ferrite_tx_demo", "ferrite_rx_demo"]
    assert len(probes) == 2
    assert [entry["kind"] for entry in entries] == ["tx_ferrite", "rx_ferrite"]
    assert modeler.objects["ferrite_rx_demo"].color == (128, 128, 128)
    assert modeler.objects["ferrite_tx_demo"].color == (128, 128, 128)
    assert all(entry["present"] is True for entry in entries)
    assert all(entry["non_model"] is False for entry in entries)
    assert entries[0]["origin_xyz"] == (9.9, -30.1, 394.2)
    assert entries[0]["size_xyz"] == (80.19999999999999, 60.2, 2.0)
    assert entries[1]["origin_xyz"] == (1.9, -40.0, 610.0)
    assert entries[1]["size_xyz"] == (2.0, 82.0, 80.0)
    tx_ferrite_top_z = entries[0]["origin_xyz"][2] + entries[0]["size_xyz"][2]
    ...
    assert modeler.subtract_calls[0]["tool_list"] == _tx_live_object_names()
    assert modeler.subtract_calls[1]["tool_list"] == _rx_live_object_names()
...
def test_create_scene_non_model_objects_uses_1mm_vertical_zone_in_mode0() -> None:
    modeler = _FakeModeler()

    _names, _probes, entries = _create_scene_non_model_objects(
        modeler=cast(Modeler3D, modeler),
        design_id="demo",
        selected=_selected(ferrite_present=True, tx_vertical_orientation_mode=0),
        selected_max=_selected_max(),
    )

    entries_by_kind = {entry["kind"]: entry for entry in entries}
    tx_dd_entry = entries_by_kind["tx_region_dd"]
    tx_vertical_entry = entries_by_kind["tx_region_vertical"]

    assert tx_vertical_entry["size_xyz"][2] == pytest.approx(1.0)
    assert tx_dd_entry["origin_xyz"][2] == pytest.approx(469.0)
    assert tx_dd_entry["origin_xyz"][2] + tx_dd_entry["size_xyz"][2] == pytest.approx(489.0)
    assert tx_vertical_entry["origin_xyz"][2] == pytest.approx(489.0)


def test_create_tx_ferrite_model_objects_raises_when_create_box_returns_false() -> None:
    modeler = _FalseCreateBoxModeler()
    hfss = _FakeHfss()

    with pytest.raises(RuntimeError, match=r"PyAEDT operation returned False: create_box"):
        _create_tx_ferrite_model_objects(
            modeler=cast(Modeler3D, modeler),
            hfss=cast(Hfss, hfss),
            design_id="demo",
...
def test_create_rx_ferrite_model_objects_raises_when_create_box_returns_false() -> None:
    modeler = _FalseCreateBoxModeler()
    hfss = _FakeHfss()
...
def test_create_split_ferrite_model_objects_record_absence_without_creating_boxes() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()
...
def test_create_rx_ferrite_model_objects_ignores_tx_yz_bboxes_when_resolving_rx_ferrite() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()

    _, _, entries = _create_rx_ferrite_model_objects(
        modeler=cast(Modeler3D, modeler),
        hfss=cast(Hfss, hfss),
        design_id="demo",
        selected=_selected(ferrite_present=False),
...
def test_create_rx_ferrite_model_objects_fails_when_rx_plate_would_leave_tv() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()
...
def test_create_tx_ferrite_model_objects_fails_when_tx_xy_fr4_is_missing() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()
...
def test_create_split_ferrite_model_objects_allows_rx_only_when_tx_ferrite_is_disabled() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()
    ...
    rx_names, rx_probes, rx_entries = _create_rx_ferrite_model_objects(
        ...
        tx_board_ids={"tx_main_0"},
        live_object_names=["coil_rx_demo", "fr4_rx_shared_yz_0_demo"],
    )

    names = tx_names + rx_names
    probes = tx_probes + rx_probes
    entries = tx_entries + rx_entries
    assert names == ["ferrite_rx_demo"]
    assert len(probes) == 1
    assert len(entries) == 1
    assert entries[0]["kind"] == "rx_ferrite"
    assert entries[0]["present"] is True


def test_create_tx_ferrite_model_objects_fails_when_tx_ferrite_touches_tx_live_object() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()
    touching_probe = cast(
        list[CadProbe],
    ...
    with pytest.raises(
        ...
        _create_tx_ferrite_model_objects(
            ...
            tx_board_ids={"tx_main_0"},
            live_object_names=["coil_tx_low_demo", "fr4_tx_main_0_xy_0_demo"],
        )


def test_create_rx_ferrite_model_objects_subtracts_finalized_rx_bridge_and_shared_fr4() -> None:
    modeler = _FakeModeler()
    hfss = _FakeHfss()

    names, probes, entries = _create_rx_ferrite_model_objects(
        modeler=cast(Modeler3D, modeler),
        hfss=cast(Hfss, hfss),
        design_id="demo",
        selected=_selected(ferrite_present=True),
        ...
        coil_plane_bboxes=_coil_plane_bboxes(),
        tx_board_ids={"tx_main_0", "tx_main_1"},
        live_object_names=["bridge_rx_dd_d_to_b_demo", "coil_rx_demo", "fr4_rx_shared_yz_0_demo"],
    )
    ...
    assert len(entries) == 1
    assert entries[0]["kind"] == "rx_ferrite"
    assert len(modeler.subtract_calls) == 1
    assert modeler.subtract_calls[0]["tool_list"] == ["bridge_rx_dd_d_to_b_demo", "coil_rx_demo", "fr4_rx_shared_yz_0_demo"]


def test_assert_tx_ferrite_gap_from_live_objects_fails_when_tx_terminal_copper_touches() -> None:
    with pytest.raises(
        ValueError,
        match=(
            "TX ferrite must keep a positive gap from TX coil copper, TX bridge objects, "
        ...
        _assert_tx_ferrite_gap_from_live_objects(
            ...
            cad_probe=cast(
                list[CadProbe],
                [{"object_name": "txs_tx_main_0_0", "bbox": [9.9, -30.1, 396.0, 90.1, 30.1, 396.6]}],
            ),
            tx_board_ids={"tx_main_0"},
        )
