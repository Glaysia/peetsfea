...
from peetsfea.legacy.type1.spec.resolver import SelectionConstraintError, resolve_selection
from peetsfea.version import SUPPORTED_SPEC_VERSION as PACKAGE_SUPPORTED_SPEC_VERSION
from tests.fixtures.legacy.type1_spec import type1_outputs_spec, write_type1_toml


def test_runnable_examples_keep_vertical_z_and_gap_defaults_in_sync() -> None:
    repo_root = Path(__file__).resolve().parents[4]
    run_spec = tomllib.loads((repo_root / "run" / "legacy" / "type1.toml").read_text(encoding="utf-8"))
    example_spec = tomllib.loads((repo_root / "examples" / "legacy" / "type1.toml").read_text(encoding="utf-8"))

    for spec in (run_spec, example_spec):
        assert spec["spec_version"] == "0.2.22"
...
def test_missing_group_geometry_section_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    start = raw.index("[coil_groups_params.neo_tx_dd.turn_count]")
    ...
    with pytest.raises(AssertionError, match="spec must contain coil_groups_params"):
...
def test_invalid_group_turn_count_range_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"coil_groups_params\.neo_tx_dd\.turn_count\.range\[0\].*must be true"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_invalid_metal_ratio_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"coil_groups_params\.neo_tx_dd\.metal_ratio must be > 0 and < 1"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_invalid_band_ratio_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"coil_groups_params\.neo_tx_dd\.band_ratio must be > 0 and < 1"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_legacy_trace_gap_keys_fail(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8")
    raw = raw.replace("[coil_groups_params.neo_tx_dd.band_ratio]", "[coil_groups_params.neo_tx_dd.trace]", 1)
    raw = raw.replace("[coil_groups_params.neo_tx_dd.metal_ratio]", "[coil_groups_params.neo_tx_dd.gap]", 1)
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("4" * 40))

    with pytest.raises(ValueError, match=r"Unknown sampled field: coil_groups_params\.neo_tx_dd\.trace"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_unsupported_spec_version_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8").replace('spec_version = "0.2.22"', 'spec_version = "0.1.6"', 1)
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("5" * 40))

    with pytest.raises(ValueError, match=r"spec_version must be '0\.2\.22'"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_removed_path_errors_on_027(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match="Removed path in spec_version 0.2.22"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_tx_vertical_span_removed_path_errors_on_027(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"Removed path in spec_version 0.2.22: coil_spacing\.tx_vertical_span_mm"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_tx_dd_top_clearance_mm_removed_path_errors_on_027(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match=r"Removed path in spec_version 0.2.22: coil_placement\.tx_dd_top_clearance_mm"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_tx_vertical_plane_removed_path_errors_on_027(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("c" * 40))
    with pytest.raises(ValueError, match=r"Removed path in spec_version 0.2.22: coil_placement\.tx_vertical_plane"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))


def test_neo_tx_dd_right_terminal_path_default_resolves(tmp_path: Path) -> None:
    toml_path = tmp_path / "neo_txdd_path.toml"
    write_type1_toml(toml_path)
    spec, _ = load_toml_bytes(toml_path)

    selected, _, _, _, _ = resolve_selection(spec, seed=1)

    assert selected["neo_tx_dd_right_terminal_path"] == "D_ccw_to_d"
    assert selected["neo_tx_dd_left_terminal_path"] == "a_cw_to_A"


def test_neo_tx_dd_left_terminal_path_default_resolves(tmp_path: Path) -> None:
    toml_path = tmp_path / "neo_txdd_left_path.toml"
    write_type1_toml(toml_path)
    spec, _ = load_toml_bytes(toml_path)
...
def test_neo_tx_dd_right_terminal_path_a_cw_to_A_resolves(tmp_path: Path) -> None:
    toml_path = tmp_path / "neo_txdd_path_a.toml"
    write_type1_toml(toml_path)
    raw = re.sub(
        r'(\[coil_placement\.neo_tx_dd_right_terminal_path\]\n(?:#.*\n)*)value = "D_ccw_to_d"',
        '\\1value = "a_cw_to_A"',
    ...
    assert selected["neo_tx_dd_right_terminal_path"] == "a_cw_to_A"


def test_neo_tx_dd_right_terminal_path_A_ccw_to_a_resolves(tmp_path: Path) -> None:
    toml_path = tmp_path / "neo_txdd_path_A.toml"
    write_type1_toml(toml_path)
    raw = re.sub(
        r'(\[coil_placement\.neo_tx_dd_right_terminal_path\]\n(?:#.*\n)*)value = "D_ccw_to_d"',
    ...
    assert selected["neo_tx_dd_right_terminal_path"] == "A_ccw_to_a"


def test_neo_tx_dd_right_terminal_path_A_ccw_to_d_resolves(tmp_path: Path) -> None:
    toml_path = tmp_path / "neo_txdd_path_A_d.toml"
    write_type1_toml(toml_path)
    raw = re.sub(
        r'(\[coil_placement\.neo_tx_dd_right_terminal_path\]\n(?:#.*\n)*)value = "D_ccw_to_d"',
        '\\1value = "A_ccw_to_d"',
    ...
    assert selected["neo_tx_dd_right_terminal_path"] == "A_ccw_to_d"


def test_invalid_neo_tx_dd_right_terminal_path_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "bad_neo_txdd.toml"
    write_type1_toml(toml_path)
    raw = re.sub(
        r'(\[coil_placement\.neo_tx_dd_right_terminal_path\]\n(?:#.*\n)*)value = "D_ccw_to_d"',
        '\\1value = "A_cw_to_B"',
    ...
    with pytest.raises(
        ValueError,
        match=r"coil_placement\.neo_tx_dd terminal path must use one uppercase and one lowercase terminal",
    ):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))


@pytest.mark.parametrize(
    ("old_header", "new_header", "expected_path"),
    [
        (
            "[coil_groups_params.neo_tx_dd.turn_count]",
        ...
        (
            ...
            r"coil_groups_params\.rx_dd\.turn_count_max",
        ),
        (
            "[coil_placement.tx_vertical_orientation_mode]",
            "[coil_placement.tx_vertical_layout_mode]",
            r"coil_placement\.tx_vertical_layout_mode",
        ),
        (
            "[coil_placement.neo_tx_dd_top_offset_ratio]",
            "[coil_placement.neo_tx_dd_top_clearance_ratio]",
            r"coil_placement\.neo_tx_dd_top_clearance_ratio",
        ),
    ],
)
def test_renamed_public_paths_fail_hard_on_0219(
    tmp_path: Path,
    ...
    toml_path = tmp_path / "renamed_path.toml"
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8").replace(old_header, new_header, 1)
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("d" * 40))

    with pytest.raises(ValueError, match=rf"Removed path in spec_version 0.2.22: {expected_path}"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_toml_basename_with_30_chars_passes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    basename = "a" * 30
    toml_path = tmp_path / f"{basename}.toml"
    write_type1_toml(toml_path)
...
def test_toml_basename_with_31_chars_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    basename = "b" * 31
    toml_path = tmp_path / f"{basename}.toml"
    write_type1_toml(toml_path)
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("9" * 40))

    with pytest.raises(ValueError, match=r"TOML basename must be <= 30 characters"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_missing_simulation_section_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(AssertionError, match="simulation must be present"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_missing_outputs_section_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(AssertionError, match="outputs must be present"):
        runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))
...
def test_invalid_adaptive_pass_constraints_fail(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    toml_path = tmp_path / "invalid_passes.toml"
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8").replace("minimum_passes = 8", "minimum_passes = 40", 1)
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("2" * 40))
