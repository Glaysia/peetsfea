...
import peetsfea.legacy.type1.spec.resolver.constraints.constraints_eval as constraints_eval
...
from peetsfea.legacy.type1.spec.resolver.types import GroupKind
...
def _txdd_layer_count(group: ResolvedCoilGroup) -> int:
...
def test_tx_vertical_span_is_derived_from_center_gap_and_count(tmp_path: Path) -> None:
    ...
    assert coil_group_selected_count(groups_by_kind["tx_vertical"]) == 6
...
def test_tx_dd_trace_width_vs_via_diameter_constraint_fails_when_trace_is_too_thin(tmp_path: Path) -> None:
    toml_path = tmp_path / "tx_dd_via_trace_fail.toml"
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8")
    ...
    raw = raw.replace(
        "[coil_groups_params.neo_tx_dd.metal_ratio]\nrange = [false, 0.5, 0.5, 1]",
        "[coil_groups_params.neo_tx_dd.metal_ratio]\nrange = [false, 0.15, 0.15, 1]",
    )
    toml_path.write_text(raw, encoding="utf-8")

    spec, _ = load_toml_bytes(toml_path)
    with pytest.raises(SelectionConstraintError, match=r"tx_dd_trace_width_vs_via_diameter"):
        resolve_selection(spec=spec, seed=1, attempt=0)


def test_tx_dd_trace_width_vs_via_diameter_constraint_passes_when_trace_is_wide_enough(tmp_path: Path) -> None:
    toml_path = tmp_path / "tx_dd_via_trace_pass.toml"
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8")
    raw = _stabilize_group_geometry_sampling(raw)
    raw = raw.replace(
    ...
    spec, _ = load_toml_bytes(toml_path)
    selected, _, _, group_geometry, _ = resolve_selection(spec=spec, seed=1, attempt=0)
    tx_dd_geometry = next(entry for entry in group_geometry if entry["kind"] == "tx_dd")
    assert float(selected["via_diameter_mm"]) == pytest.approx(0.5)
    assert float(tx_dd_geometry["trace"]) >= float(selected["via_diameter_mm"])
...
def test_one_turn_selection_succeeds_for_all_groups_with_two_layer_txdd(tmp_path: Path) -> None:
    ...
    groups_by_kind = {group["kind"]: group for group in groups}
    turns_by_kind = {entry["kind"]: int(entry["turn_count"]) for entry in group_geometry}

    assert int(groups_by_kind["tx_dd"]["layer_count"]) == 1
...
def test_one_turn_selection_succeeds_for_all_groups_with_four_layer_txdd(tmp_path: Path) -> None:
    ...
    write_type1_toml(toml_path)
    raw = _stabilize_group_geometry_sampling(toml_path.read_text(encoding="utf-8"))
    raw = raw.replace("stacked_mode = [true, 0, 1, 2]", "stacked_mode = [true, 1, 1, 1]")
    raw = raw.replace("count_range = [true, 1, 6, 6]", "count_range = [true, 1, 1, 1]")
    toml_path.write_text(raw, encoding="utf-8")
    ...
    assert int(groups_by_kind["tx_dd"]["layer_count"]) == 2
    assert turns_by_kind == {"tx_dd": 1, "tx_vertical": 1, "rx_dd": 1}
...
def test_tx_bridge_right_margin_rule_blocks_case_that_center_only_logic_would_allow(tmp_path: Path) -> None:
    ...
    raw = raw.replace(
        "[[coil_groups]]\nkind = \"tx_dd\"\nstacked_mode = [true, 0, 0, 1]\ninstance_transforms = [{ dx = 0.0, dy = 0.0, dz = 0.0, rot_deg = 0.0 }]",
        "[[coil_groups]]\nkind = \"tx_dd\"\nstacked_mode = [true, 0, 0, 1]\ninstance_transforms = [{ dx = 0.0, dy = -10.0, dz = 0.0, rot_deg = 0.0 }]",
    )
    toml_path.write_text(raw, encoding="utf-8")
    ...
    tx_dd_layer_count = _txdd_layer_count(tx_dd_group)
    tx_dd_instance_count = 2 if tx_dd_layer_count == 1 else 4
    ...
    tx_vertical_selected_count = coil_group_selected_count(tx_vertical_group)
    for pcb in pcbs:
        if not pcb["present"]:
            continue
        for instance_index in range(tx_vertical_selected_count):
            ...
            off_y = constraints_eval._tx_vertical_instance_offset_y(
                instance_index=instance_index,
                instance_count=tx_vertical_selected_count,
