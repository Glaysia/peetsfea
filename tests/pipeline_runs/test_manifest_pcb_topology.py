...
def test_relative_z_mode_requires_base_and_delta_path(tmp_path: Path) -> None:
    ...
    toml_path.write_text(raw, encoding="utf-8")
    spec, _ = load_toml_bytes(toml_path)
    with pytest.raises(ValueError, match=r"z_relative_base_id and z_delta_path must be set when z_mode='relative_to_pcb'"):
        resolve_selection(spec=spec, seed=1, attempt=0)
...
def test_derived_path_requires_dummy_range_literal(tmp_path: Path) -> None:
    ...
    toml_path.write_text(raw, encoding="utf-8")
    spec, _ = load_toml_bytes(toml_path)
    with pytest.raises(ValueError, match=r"coil_shape\.neo_tx_vertical\.outer_x\.range for derived path must be exactly"):
        resolve_selection(spec=spec, seed=1, attempt=0)
...
def test_tx_dd_single_layer_strips_upper_board_mounts_after_normalization(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    toml_path = tmp_path / "txdd_single_layer.toml"
    write_type1_toml(toml_path)
    raw = toml_path.read_text(encoding="utf-8")
    raw = raw.replace("stacked_mode = [true, 0, 1, 2]", "stacked_mode = [true, 0, 0, 1]", 1)
    toml_path.write_text(raw, encoding="utf-8")
    monkeypatch.setattr(runner, "get_git_commit", lambda _: ("f" * 40))

    manifest = runner.run(runner.RunConfig("/bin/ansysedt", str(tmp_path), str(toml_path), seed=1, backend="hfss"))["manifest"]
    pcbs_by_id = {pcb["id"]: pcb for pcb in manifest["selected_pcbs"]}

    tx_main_0_mounts = pcbs_by_id["tx_main_0"]["mounts"]
    tx_main_1_mounts = pcbs_by_id["tx_main_1"]["mounts"]

    assert [mount["selector_index"] for mount in tx_main_0_mounts if mount["kind"] == "tx_dd"] == [0, 1]
    assert [mount for mount in tx_main_1_mounts if mount["kind"] == "tx_dd"] == []
