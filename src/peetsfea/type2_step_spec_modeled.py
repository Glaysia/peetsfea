...
def _require_tx_inner_underlay_thickness_range(
    table: dict[str, object],
    *,
    key: str,
    context: str,
    ...
    if range_spec.count != 1:
        raise ValueError(
            f"{context}.{key} must be fixed [false, t, t, 1] with t > 0 for tx_inner_single_coil "
            f"(actual={[range_spec.is_integer, range_spec.start, range_spec.end, range_spec.count]})"
        )
...
def _parse_modeled_single_coil(
    ...
    *,
    ...
    tx_inner_allowed_keys |= {"underlay_pet_psa_thickness_mm", "underlay_ferrite_thickness_mm"}
    tx_inner_allowed_keys |= {"tx_outer_x_position_ratio"}
    ...
    if modeled_role == "tx_inner_single_coil":
        tx_outer_x_position_ratio = _require_x_position_ratio_range(
            table,
            "tx_outer_x_position_ratio",
            context=context,
        )
        ...
        underlay_pet_psa_thickness_mm = _require_tx_inner_underlay_thickness_range(
            table,
            key="underlay_pet_psa_thickness_mm",
            context=context,
        )
        underlay_ferrite_thickness_mm = _require_tx_inner_underlay_thickness_range(
            table,
        ...
        return ModeledTxInnerSingleCoilSpec(
            ...
            metal_fill_factor=metal_fill_factor,
            terminal_path=terminal_path,
            tx_outer_x_position_ratio=tx_outer_x_position_ratio,
        )
    if "underlay_gap_mm" in table:
...
def _parse_tx_outer_x_position_ratio_selector(raw_object: object, *, index: int) -> RangeSpec:
    context = f"modeled_objects[{index}]"
    table = _require_table(raw_object, context)
    role = _require_non_empty_str(table, "role", context)
    if "tx_outer_x_position_ratio" not in table:
        raise ValueError(f"{context}.tx_outer_x_position_ratio selector is missing")
    if role != "tx_inner_single_coil":
        raise ValueError(
            f"{context}.tx_outer_x_position_ratio is supported only on tx_inner_single_coil "
            f"(actual_role={role!r})"
        )
    return _require_x_position_ratio_range(table, "tx_outer_x_position_ratio", context=context)
...
def append_tx_outer_single_coil_companion_specs(
    ...
    x_position_selector_indexes = tuple(
        index
        for index, raw_object in enumerate(raw_modeled_objects)
        if "tx_outer_x_position_ratio" in _require_table(raw_object, f"modeled_objects[{index}]")
    )
    if len(selector_indexes) == 0 and len(x_position_selector_indexes) == 0:
    ...
    if len(x_position_selector_indexes) != 1:
        raise ValueError(
            "tx_outer_x_position_ratio selector must appear exactly once when deriving tx_outer_single_coil "
            f"(actual={len(x_position_selector_indexes)})"
        )
    selector_index = selector_indexes[0]
    x_position_selector_index = x_position_selector_indexes[0]
    if x_position_selector_index != selector_index:
        raise ValueError(
            "tx_outer_terminal_path and tx_outer_x_position_ratio selectors must be on the same "
            "tx_inner_single_coil object"
        )
    terminal_path = _parse_tx_outer_terminal_path_selector(raw_modeled_objects[selector_index], index=selector_index)
    _ = _parse_tx_outer_x_position_ratio_selector(
        raw_modeled_objects[x_position_selector_index],
        index=x_position_selector_index,
    )
    ...
    companion = ModeledTxOuterSingleCoilSpec(
        ...
        x_position_ratio=tx_inner_spec.tx_outer_x_position_ratio,
        ...
        underlay_pet_psa_thickness_mm=tx_inner_spec.underlay_pet_psa_thickness_mm,
        underlay_ferrite_thickness_mm=tx_inner_spec.underlay_ferrite_thickness_mm,
    )
