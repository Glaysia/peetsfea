...
from peetsfea.types.manifest import ResolvedCoilGroup, ResolvedPcbInstance, ResolvedPcbMount
...
def parse_pcb_mounts(value: TOMLValue, name: str) -> list[ResolvedPcbMount]:
    ...
    for idx, entry in enumerate(value):
        ...
        if "kind" not in entry or "selector_mode" not in entry:
            raise ValueError(f"{name}[{idx}] must contain kind and selector_mode")
        kind_raw = entry["kind"]
        if kind_raw not in GROUP_KIND_ORDER:
            raise ValueError(f"{name}[{idx}].kind must be one of {list(GROUP_KIND_ORDER)}")
        selector_mode_raw = entry["selector_mode"]
        ...
        if selector_mode == "all":
            if "selector_index" in entry:
                raise ValueError(f"{name}[{idx}].selector_index must be omitted when selector_mode='all'")
            selector_index = None
        else:
            if "selector_index" not in entry:
                raise ValueError(f"{name}[{idx}].selector_index must be set when selector_mode='index'")
            selector_index_raw = entry["selector_index"]
...
def resolve_pcbs(spec: TOMLTable, seed: int, attempt: int, context: SamplingContext) -> list[ResolvedPcbInstance]:
    if "pcbs" not in spec or not isinstance(spec["pcbs"], list) or len(spec["pcbs"]) == 0:
    ...
    for idx, raw_pcb in enumerate(raw_pcbs):
        ...
        if "id" not in raw_pcb_table or "role" not in raw_pcb_table:
            raise ValueError(f"pcbs[{idx}] must contain id and role")
        raw_id = raw_pcb_table["id"]
        raw_role = raw_pcb_table["role"]
        ...
        if "position" not in raw_pcb_table or "rotation_deg" not in raw_pcb_table or "mounts" not in raw_pcb_table:
            raise ValueError(f"pcbs[{idx}].position must be [x, y, z]")
        raw_position = raw_pcb_table["position"]
        position = parse_position(raw_position, f"pcbs[{idx}].position")
        raw_rotation = raw_pcb_table["rotation_deg"]
        ...
        raw_mounts = raw_pcb_table["mounts"]
        mounts = parse_pcb_mounts(raw_mounts, f"pcbs[{idx}].mounts")
        if "present" not in raw_pcb_table or not isinstance(raw_pcb_table["present"], list) or len(raw_pcb_table["present"]) != 4:
        ...
        if z_mode == "absolute":
            if "z_relative_base_id" in raw_pcb_table or "z_delta_path" in raw_pcb_table:
                raise ValueError(
                    f"pcbs[{idx}] absolute z_mode must not set z_relative_base_id or z_delta_path"
                )
        else:
            if "z_relative_base_id" not in raw_pcb_table or "z_delta_path" not in raw_pcb_table:
                raise ValueError(
                    f"pcbs[{idx}].z_relative_base_id and z_delta_path must be set when z_mode='relative_to_pcb'"
                )
            raw_z_relative_base_id = raw_pcb_table["z_relative_base_id"]
            raw_z_delta_path = raw_pcb_table["z_delta_path"]
    ...
    for idx, resolved_pcb in enumerate(resolved):
        ...
        assert isinstance(base_id, str), f"pcbs[{idx}] relative_to_pcb requires z_relative_base_id"
        assert isinstance(delta_path, str), f"pcbs[{idx}] relative_to_pcb requires z_delta_path"
...
def _tx_dd_layer_count(groups: list[ResolvedCoilGroup]) -> int:
    for group in groups:
        if group["kind"] == "tx_dd":
            return int(group["layer_count"])
    raise ValueError("spec_version fixed topology requires selected tx_dd group")


def _effective_fixed_mounts(
    *,
    pcb_id: str,
    groups: list[ResolvedCoilGroup],
) -> tuple[PcbMountSpec, ...]:
    if pcb_id != "tx_main_1":
        return FIXED_PCB_RULES[pcb_id]["mounts"]
    if _tx_dd_layer_count(groups) == 1:
        return ()
    return FIXED_PCB_RULES[pcb_id]["mounts"]


def normalize_pcbs_fixed_topology(
    ...
    for pcb_id in FIXED_PCB_ORDER:
        ...
        expected_mounts = _effective_fixed_mounts(pcb_id=pcb_id, groups=groups)
