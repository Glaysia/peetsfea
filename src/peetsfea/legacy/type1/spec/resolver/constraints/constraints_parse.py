...
def parse_rule(raw_rule: TOMLValue, idx: int) -> ConstraintRule:
    ...
    if raw_kind == "range":
        ...
        has_min = "min" in table
        has_max = "max" in table
        min_ref = parse_value_ref(table["min"], f"{dotted}.min") if has_min else None
        max_ref = parse_value_ref(table["max"], f"{dotted}.max") if has_max else None
        if not has_min and not has_max:
            raise ValueError(f"{dotted} must define at least one of min/max")
        inclusive_min = True
        inclusive_max = True
        if "inclusive_min" in table:
            inclusive_min = table["inclusive_min"]
        if "inclusive_max" in table:
            inclusive_max = table["inclusive_max"]
...
def parse_constraints(spec: TOMLTable) -> list[ConstraintRule]:
    assert "constraints" in spec, "spec must contain constraints"
    constraints = require_table(spec["constraints"], "constraints")
    assert "rules" in constraints, "constraints must contain rules"
    raw_rules = constraints["rules"]
