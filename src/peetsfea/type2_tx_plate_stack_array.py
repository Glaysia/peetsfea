...
def build_tx_plate_stack_array_scene_data(
    ...
    *,
    ...
    branch_copper_shapes: list[Shape] = []
    branch_pcb_shapes: list[Shape] = []
    branch_ferrite_shapes: list[Shape] = []
    ...
    for index in range(tx_coil_count):
        ...
        if "tx_plate_copper" in branch_shape_by_label:
            ...
            copper_children = tuple(cast(Shape, child) for child in copper_group_shape.children)
        ...
        ferrite_children = tuple(cast(Shape, child) for child in ferrite_group_shape.children)
        ...
        branch_pcb_shapes.extend(
            (
                _cut_tx_branch_pcb_blank_with_ferrite_family_tools(
                    blank_shape=branch_pcb_wall_shape,
                    ferrite_family_tools=(
                        branch_ferrite_pet_psa_shape,
                        branch_ferrite_shape,
                        branch_ferrite_air_shape,
                    ),
                ...
                _cut_tx_branch_pcb_blank_with_ferrite_family_tools(
                    blank_shape=branch_pcb_coil_shape,
                    ferrite_family_tools=(
                        branch_ferrite_pet_psa_shape,
                        branch_ferrite_shape,
                        branch_ferrite_air_shape,
    ...
    input_connector_shapes: list[Shape] = []
    output_connector_shapes: list[Shape] = []
    ...
    top_level_shapes: list[Shape] = [cast(Shape, fused_copper_solid)]
    top_level_shapes.extend(branch_pcb_shapes)
    top_level_shapes.append(cast(Shape, ferrite_group_shape))
...
def _cut_tx_branch_pcb_blank_with_ferrite_family_tools(
    *,
    blank_shape: Shape,
    ferrite_family_tools: tuple[Shape, ...],
    branch_index: int,
) -> Shape:
    blank_label = blank_shape.label
    if blank_label not in (f"tx_b{branch_index}_pcb_wall", f"tx_b{branch_index}_pcb_coil"):
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance received unexpected PCB blank "
            f"(branch_index={branch_index}, blank_label={blank_label})"
        )
    if len(ferrite_family_tools) != 3:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance requires grouped ferrite-family tools "
            f"(branch_index={branch_index}, blank_label={blank_label}, tool_count={len(ferrite_family_tools)})"
        )
    expected_tool_labels = (
        f"tx_b{branch_index}_stack_pet_psa",
        f"tx_b{branch_index}_stack_ferrite",
        f"tx_b{branch_index}_stack_air",
    )
    actual_tool_labels = tuple(tool.label for tool in ferrite_family_tools)
    if actual_tool_labels != expected_tool_labels:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance tool label/order drifted "
            f"(branch_index={branch_index}, blank_label={blank_label}, "
            f"expected={expected_tool_labels}, actual={actual_tool_labels})"
        )
    blank_solids = tuple(blank_shape.solids())
    if len(blank_solids) != 1:
        raise RuntimeError(
            "type2 tx array PCB blank must contain exactly one solid before ferrite-priority clearance "
            f"(branch_index={branch_index}, blank_label={blank_label}, solid_count={len(blank_solids)})"
        )
    blank_volume = blank_solids[0].volume
    if blank_volume <= 0.0:
        raise RuntimeError(
            "type2 tx array PCB blank must have positive volume before ferrite-priority clearance "
            f"(branch_index={branch_index}, blank_label={blank_label}, volume={blank_volume})"
        )
    for tool_shape in ferrite_family_tools:
        tool_solids = tuple(tool_shape.solids())
        if len(tool_solids) != 1:
            raise RuntimeError(
                "type2 tx array ferrite-priority tool must contain exactly one solid "
                f"(branch_index={branch_index}, blank_label={blank_label}, "
                f"tool_label={tool_shape.label}, solid_count={len(tool_solids)})"
            )
        if tool_solids[0].volume <= 0.0:
            raise RuntimeError(
                "type2 tx array ferrite-priority tool must have positive volume "
                f"(branch_index={branch_index}, blank_label={blank_label}, "
                f"tool_label={tool_shape.label}, volume={tool_solids[0].volume})"
            )
    tool_compound = bd.Compound(
        children=tuple(ferrite_family_tools),
        label=f"tx_b{branch_index}_ferrite_clearance_tools",
    )
    raw_cut_shape_or_list = blank_shape.cut(tool_compound)
    assert isinstance(raw_cut_shape_or_list, (Shape, ShapeList))
    cut_shape_or_list: Shape | ShapeList[Shape] = raw_cut_shape_or_list
    if isinstance(cut_shape_or_list, ShapeList):
        if len(cut_shape_or_list) != 1:
            raise RuntimeError(
                "type2 tx array ferrite-priority clearance must resolve one PCB blank shape "
                f"(branch_index={branch_index}, blank_label={blank_label}, shape_count={len(cut_shape_or_list)})"
            )
        cut_shape = cast(Shape, cut_shape_or_list[0])
    else:
        cut_shape = cut_shape_or_list
    cut_solids = tuple(cut_shape.solids())
    if len(cut_solids) != 1:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance must resolve one PCB blank solid "
            f"(branch_index={branch_index}, blank_label={blank_label}, solid_count={len(cut_solids)})"
        )
    cut_solid = cut_solids[0]
    if cut_solid.volume <= 0.0:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance produced non-positive PCB blank volume "
            f"(branch_index={branch_index}, blank_label={blank_label}, volume={cut_solid.volume})"
        )
    if cut_solid.volume > blank_volume + 1e-9:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance increased PCB blank volume "
            f"(branch_index={branch_index}, blank_label={blank_label}, "
            f"before_volume={blank_volume}, after_volume={cut_solid.volume})"
        )
    cut_solid.label = blank_label
    if cut_solid.label != blank_label:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance blank label drifted "
            f"(branch_index={branch_index}, expected={blank_label}, actual={cut_solid.label})"
        )
    post_tool_labels = tuple(tool.label for tool in ferrite_family_tools)
    if post_tool_labels != expected_tool_labels:
        raise RuntimeError(
            "type2 tx array ferrite-priority clearance mutated ferrite-family tool labels "
            f"(branch_index={branch_index}, expected={expected_tool_labels}, actual={post_tool_labels})"
        )
    return cast(Shape, cut_solid)
...
def _labeled_bridge_connector_body(
    *,
    ...
    return cast(Shape, bridge_solid)
...
def _build_labeled_united_copper_body(*, label: str, source_shapes: tuple[Shape, ...]) -> Shape:
    ...
    fused_shape_or_list: Shape | ShapeList[Shape] = source_shapes[0]
    for source_shape in source_shapes[1:]:
        fuse_base_shape = (
            cast(Shape, bd.Compound(children=tuple(fused_shape_or_list)))
            if isinstance(fused_shape_or_list, ShapeList)
