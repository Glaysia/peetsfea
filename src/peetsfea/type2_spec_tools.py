...
def _owner_range_field_table(
    ...
    *,
    ...
    source_owner_path = _raw_toml_owner_path(owner_path)
    owner_parts = source_owner_path.split(".")
    if len(owner_parts) < 3:
        raise ValueError(f"Unsupported type2 sampled owner path: {owner_path} (source={source_owner_path})")
    ...
    for table_name in owner_parts[2:-1]:
        if table_name not in owner_table:
            raise ValueError(
                f"type2 source TOML is missing sampled owner table for {owner_path}: {source_owner_path}"
            )
        raw_owner_table = owner_table[table_name]
        if not isinstance(raw_owner_table, dict):
            raise TypeError(f"{owner_path} source parent {table_name!r} must be a table: {source_owner_path}")
        owner_table = cast(TOMLTable, raw_owner_table)
    field_name = owner_parts[-1]
    if field_name not in owner_table:
        raise ValueError(f"type2 source TOML is missing sampled owner field for {owner_path}: {source_owner_path}")
    raw_field = owner_table[field_name]
    if not isinstance(raw_field, dict):
        raise TypeError(f"{owner_path} source field must be a table containing range: {source_owner_path}")
