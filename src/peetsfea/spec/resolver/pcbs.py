...
def normalize_pcbs_fixed_topology(pcbs: list[ResolvedPcbInstance]) -> list[ResolvedPcbInstance]:
    ...
    if missing or extra:
        ...
        raise ValueError(
            "spec_version 0.2.4 requires fixed pcbs topology ids "
    ...
    for pcb_id in FIXED_PCB_ORDER:
        ...
        if pcb["role"] != rule["role"]:
            raise ValueError(
                "spec_version 0.2.4 fixed topology requires "
        ...
        if pcb["present"] != expected_present:
            warnings.warn(
                f"pcbs.{pcb_id}.present normalized to {expected_present} for spec_version 0.2.4 fixed topology",
                UserWarning,
                stacklevel=2,
            )
        ...
        if mount_specs(pcb["mounts"]) != expected_mounts:
            warnings.warn(
                f"pcbs.{pcb_id}.mounts normalized to fixed topology mapping for spec_version 0.2.4",
                UserWarning,
                stacklevel=2,
            )
