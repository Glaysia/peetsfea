...
class ImportedTxOuterCanonicalTiltMetadata(TypedDict, total=False):
    ...
    # Maximum accepted underhang below the outer region in world -Z for rigid-tilt stack placement.
    max_world_z_underhang_mm: float
...
def _parse_outer_tilt_metadata(value: object, *, context: str) -> ImportedTxOuterCanonicalTiltMetadata:
    ...
    allowed_keys = ("max_world_x_protrusion_mm", "max_world_z_underhang_mm")
    ...
    return {
        ...
        "max_world_z_underhang_mm": _require_positive_float_or_zero(
            _require_key(node, key="max_world_z_underhang_mm", context=context),
