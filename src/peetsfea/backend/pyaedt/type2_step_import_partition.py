...
def resolve_modeled_body_names(
    *,
    ...
    if len(imported_object_names) != len(expected_names):
        raise ValueError(
            "single-coil type2 import requires imported body count to match the expected exported body count "
            f"(expected={len(expected_names)}, actual={len(imported_object_names)})"
        )
    ...
    pcb_names = [name for name in imported_object_names if _body_role_from_expected_name(name, context=context) == _BODY_ROLE_PCB]
    copper_names = [
        name for name in imported_object_names if _body_role_from_expected_name(name, context=context) == _BODY_ROLE_COPPER
    ]
